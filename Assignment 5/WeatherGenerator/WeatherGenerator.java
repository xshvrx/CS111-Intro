...
public class WeatherGenerator {
    ...
    public static int[] oneMonthGenerator(double longitude, double latitude, int month, double[][] drywet, double[][] wetwet) {
        ...
        int numRows = drywet.length;
        double dryToWetProb = 0.0;
        double wetToWetProb = 0.0;
        for (int x = 0; x < numRows; x++) {
            if (drywet[x][0] == longitude && wetwet[x][1] == latitude) {
                dryToWetProb = drywet[x][month];
                wetToWetProb = wetwet[x][month];
            }
        }
        ...
        for (int i = 0; i < forecast.length; i++) {
            if (i == 0) {
                if (random >= 0 && random <= 0.49) 
                    forecast[i] = WET;
                else if (random >= 0.50 && random <= 0.99) 
                    forecast[i] = DRY;
            } else {
                if (forecast[i-1] == WET){
                    random = StdRandom.uniform();
                    if (random <= wetToWetProb) 
                        forecast[i] = WET;
                    else
                        forecast[i] = DRY; 
                }
